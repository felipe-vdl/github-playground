  ...
  return (
    <div className="bg-slate-800 min-h-screen bg-gradient-to-br from-rose-900/20 to-blue-900/10 flex flex-col">
      ...
      <main className="overflow-hidden flex-1 p-8 flex flex-col gap-8 text-white/75 relative">
        {cartIsOpen && <>
          <div className={`bg-slate-800 drop-shadow absolute top-0 right-0 h-full z-20 flex flex-col ${cartIsClosing ? "animation-slide-out" : "animation-slide-in"}`}>
            ...
            <h2 className="italic text-center text-indigo-400 font-bold my-3">Total: ${cartProducts.reduce((acc, product) => product.price * product.quantity + acc, 0) / 100}</h2>
          </div>
          <div onClick={handleClick} className={`top-0 left-0 z-10 backdrop-blur-[1px] w-full h-full bg-black/30 absolute ${cartIsClosing ? "animation-fade-out" : "animation-fade-in"}`} />
        </>}
          ...
          {storeProducts.map(product =>
            <div className="flex justify-between border-b border-white/20 py-4" key={`${product.id}-${product.description.charAt(0)}`}>
              ...
              <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                ...
                <button className="rounded-full bg-indigo-500 p-1 hover:bg-indigo-600 transition-all duration-200">+ Add</button>
              </form>
            </div>
          )}
        </section>
